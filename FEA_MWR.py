...
x0,y0,y1,x1,phi0,phix,phiy,phi, N = sym.symbols('x_0 y_0 y_1 x_1 phi_0 phi_x phi_y phi N')
...
trial_function = (1 - x) * (1 - y)
dphi_dx = sym.Derivative(phi,x)
dphi_dy = sym.Derivative(phi,y)
dN_dx = sym.Derivative(N,x)
dN_dy = sym.Derivative(N,y)

weak_form_equation = -sym.Integral(sym.Integral( dphi_dx * dN_dx + dphi_dy * dN_dx ,x),y) + sym.Integral(sym.Integral( N * f ,x),y)

print(sym.latex(weak_form_equation))
